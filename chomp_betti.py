...
def stack_images( path, num_frames=50 ):
    ...
    for i,x in enumerate( frames ):
        #tmp = numpy.empty( (x.shape[0], len( frames )), dtype=numpy.int )
        tmp = numpy.empty( (x.shape[0], 1), dtype=numpy.int )
        tmp.fill( i )
        frames[i] = numpy.hstack( (x, tmp) )
    return numpy.vstack( frames )

def stack_frames_fft( frame_path, nstart=100, num_frames=5 ):
    """
    <path> points to a directory containing pickled files. Each files
    containing all of the saved FFT data for a single frame of a given
    cell, calculated a a given bandwidth cutoff. Eg.,

    path=/data/jberwald/rbc/New/frames/new_140125/normed_frames/r00/cell_name_num.pkl

    boundary should point to the boundary file associated to each
    cell. Eg.,

    boundary=/data/jberwald/rbc/New/boundary_Nov_new140125

    num_frames : frames to read from cell file, starting from nstart
    Default==5. nstart Default==0. A value of num_frames==-1 indicates
    read all frames. Careful, could take awhile.

    Returns a 3D numpy array.
    """
    if not frame_path.endswith( '/' ): frame_path += '/'
    dlist = os.listdir( frame_path )
    dlist.sort( key=natural_key )
    # read in only (nstart, nstart+num_frames)
    # assumes directory contains only frames!!
    frame_list = dlist[ nstart : nstart+num_frames ]

    print "frames", frame_list
    
    # extract the thresholded ifft data for each frame in frame_nums
    fft_frames = []
    for f in frame_list:
        with open( frame_path + f ) as fh:
            fft_frames.append( pkl.load( fh )['ifft_nonzero'] )
    return fft_frames
    #    arr = numpy.asarray( im )
    # Find where pixels are black. 255 == white. 
    # w = numpy.where( arr != 255 )    
